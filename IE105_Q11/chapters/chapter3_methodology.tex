...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Real-time Monitoring:} Dashboard liên tục truy vấn tệp \texttt{state/metrics.json} (polling) để vẽ biểu đồ diễn biến của Loss và Accuracy.
...
\begin{enumerate}
    ...
    $$ \text{Input (55)} \xrightarrow{\text{Linear}} \text{Hidden (64)} \xrightarrow{\text{ReLU}} \text{Hidden (16)} \xrightarrow{\text{ReLU}} \text{Latent (4)} \xrightarrow{\text{Tanh}} \text{Latent' (4)} \xrightarrow{\times \pi} \text{Rotation Angles} $$
...
Việc tích hợp Opacus biến trình tối ưu hóa thông thường (Adam/SGD) thành \texttt{DPOptimizer}, đảm bảo các đảm bảo toán học về sự riêng tư được thực thi nghiêm ngặt trong mỗi bước huấn luyện cục bộ.
