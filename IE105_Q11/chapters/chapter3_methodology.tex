...
Việc tích hợp Opacus \cite{yousefpour2021opacus} biến trình tối ưu hóa thông thường (Adam/SGD) thành \path{DPOptimizer}, đảm bảo các đảm bảo toán học về sự riêng tư được thực thi nghiêm ngặt trong mỗi bước huấn luyện cục bộ.
...
\begin{enumerate}
    ...
    Bên cạnh Median và Krum, hệ thống hỗ trợ chiến lược \textbf{Bulyan} \cite{mhamdi2018byzantine} và \textbf{Median-of-Means (MoM)} \cite{lerasle2011robust} nhằm nâng cao khả năng chống chịu tấn công đầu độc Byzantine từ các client độc hại.
    ...
    Hỗ trợ thuật toán \textbf{FedProx} \cite{li2020federated} (\texttt{--fedprox-mu}) bổ sung số hạng phạt proximal term để xử lý hiện tượng không đồng nhất (heterogeneity) của dữ liệu Non-IID; đồng thời mở rộng đo lường đầy đủ các chỉ số an ninh mạng quan trọng gồm \textbf{ROC-AUC}, \textbf{PR-AUC} và \textbf{Confusion Matrix} (ma trận nhầm lẫn).
\end{enumerate}