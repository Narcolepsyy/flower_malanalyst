...
\textbf{Kết luận:} Trong thử nghiệm của chúng tôi, Federated Learning cho thấy kết quả khả quan hơn so với huấn luyện tập trung đối với mô hình Hybrid QNN. Tuy nhiên, kết quả centralized ($\approx 93.75\%$) được ghi nhận từ một lần chạy duy nhất trên tệp thử nghiệm lượng tử tại \path{notebooks/quantum_experiments.ipynb} ngoài pipeline chính. Cần thêm các thí nghiệm với nhiều lần chạy (mean$\pm$std) và cùng seed để có thể khẳng định chắc chắn xu hướng này.
...
\begin{enumerate}
    ...
    \item \textbf{Đánh giá Krum:} Krum đạt độ chính xác $99.83\%$, tương đương với FedAvg và Median. Điều này phù hợp với kỳ vọng lý thuyết khi chạy thực nghiệm trong điều kiện phân phối dữ liệu IID sạch không có tấn công, khiến ba chiến lược hội tụ về cùng một kết quả tối ưu.
\end{enumerate}
...
\textit{\textbf{Lưu ý về tái lập:} Số liệu trong bảng được trích xuất trực tiếp từ kết quả giải thích mô hình toàn cục Logistic Regression ở vòng thứ 5 (vòng hội tụ), hiện được lưu trữ đầy đủ trong tệp tin \path{state/explanations.json}. Lệnh thực thi để tạo ra kết quả này là:}
\begin{center}
